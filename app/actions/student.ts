...
const StudentSchema = z.object({
  ...
  phone: z.string().regex(/^\d{10}$/, "Invalid phone number (exactly 10 digits)").optional().or(z.literal("")),
  bloodGroup: z.string().optional(),
  address: z.string().min(5, "Address must be descriptive"),
  pincode: z.string().regex(/^\d{6}$/, "Invalid pincode (must be exactly 6 digits)"),
...
export async function addStudent(formData: FormData) {
  ...
  const newStudent = {
    ...
    bloodGroup:        validated.data.bloodGroup || getString("bloodGroup"),
