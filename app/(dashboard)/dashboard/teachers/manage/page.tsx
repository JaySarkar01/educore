  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-fg tracking-tight">Manage Teachers</h1>
          <p className="text-muted-fg mt-1 text-lg">Manage faculty employment details and subject assignments.</p>
        </div>
      ...
      <Card className="shadow-lg shadow-brand-500/5 overflow-hidden border-border/50">
        {/* Simple Filter Bar implementation directly without external component for speed */}
        <div className="p-4 border-b border-border/40 bg-surface-50 dark:bg-surface-900/50 flex flex-wrap gap-3">
          <form className="flex gap-2 w-full sm:w-auto">
            <input name="q" defaultValue={q} placeholder="Search by name, ID, email..." className="flex h-9 w-full sm:w-64 rounded-md border border-input bg-surface-50 dark:bg-surface-950 px-3 py-1 text-sm shadow-sm" />
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-border/40">
              {teachers.length === 0 ? (
                ...
                <tr key={t._id} className="hover:bg-surface-100/50 dark:hover:bg-surface-900/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="font-semibold text-fg text-base">{t.name}</div>
                    <div className="text-xs text-muted-fg mt-0.5">{t.qualification} • {t.experience} Yrs Exp</div>
                  </td>
                  <td className="px-6 py-4 font-medium text-fg">{t.employeeId}</td>
                  <td className="px-6 py-4 text-fg">{t.department || 'General'}</td>
                  <td className="px-6 py-4">
