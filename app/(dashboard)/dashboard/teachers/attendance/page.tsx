...
export default async function TeacherAttendancePage(
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <div>
        <h1 className="text-xl md:text-2xl lg:text-3xl font-bold text-fg tracking-tight">Faculty Attendance</h1>
        <p className="text-muted-fg mt-1 text-sm md:text-base">Mark daily check-ins for all active and deployed staff members.</p>
      </div>

      <div className="bg-surface-50 dark:bg-surface-950 border border-border/50 rounded-xl p-4 md:p-6 shadow-sm flex flex-col sm:flex-row items-end gap-4">
        <form className="flex-1 max-w-sm space-y-2 relative">
          ...
          <div className="flex gap-3 items-center">
            ...
            <Button type="submit" className="h-10 bg-brand-600 hover:bg-brand-700 text-white shadow-sm shadow-brand-500/20 whitespace-nowrap"><Filter className="w-4 h-4 mr-2"/>Query</Button>
          </div>
        </form>
      </div>
